...
public class ModifiedURI implements Cloneable, Comparable, Serializable {
  ...
  protected int indexFirstOf(String s, String delims, int offset) {
    ...
    for (char element : delim) {
      int at = s.indexOf(element, offset);
      if (at >= 0 && at < min) {
        min = at;
      }
    }
